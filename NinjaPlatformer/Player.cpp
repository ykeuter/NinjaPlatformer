...
void Player::init(b2World* world,
    ...
    GameEngine::GLTexture texture = GameEngine::ResourceManager::getTexture("Assets/blue_ninja.png");
    m_color = color;
    m_drawDims = drawDims;
    ...
    for (b2ContactEdge* ce = body->GetContactList(); ce != nullptr; ce = ce->next) {
        ...
        if (c->IsTouching()) {
            ...
            for (int i = 0; i < b2_maxManifoldPoints; i++) {
                if (manifold.points[i].y < body->GetPosition().y - m_capsule.getDimensions().y / 2.0f +
                    m_capsule.getDimensions().x / 2.0f + 0.01f) {
                    below = true;
                    break;
                }
            }
